...
%%
function [iNO_end, iEL_end] = MGT_side8(fileID, iNO, iEL, CoC_side, facade_side_R, levelZaxis, levelPstart, Roof_boundary, ~, ~, ROOF, Arc_itvl)
%% NODE
...
for i = 1:lengthlevelZaxis	%
    for j = 1:(sideColu_num-1)
        iN1_bkp = iNO+2+j+lengthXYcoor2*(i-1); % 内部点
        iN2_bkp = iN1_bkp+1;
...
for i = levelPstart:lengthlevelZaxis	%
    for j = 1:(sideColu_num-1)
        iN1_bkp = iNO+2+sideColu_num+j+lengthXYcoor2*(i-1); %
        iN2_bkp = iN1_bkp+1;
