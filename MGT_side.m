...
switch tower_num % 塔标号
    case 7
        ...
        sideRadius = 3250; sideColu_num = 4;
        ...
        [X_temp, Y_temp, ~] = coorLxC_sym(CoC_side, sideRadius, Corner_coor, Roof_boundary(5,:)); %左边第一点 % [X, Y, Len] = coorLxC_sym(C0, R, P1, P2);
        if X_temp(1) > X_temp(2)
        ...
        for i = levelPstart:lengthlevelZaxis % levelPstart以下都为0
            for j = 1:sideColu_num
                [X_temp, Y_temp, ~] = coorLxC_sym(CoC_side, facade_stair_R(i), Corner_coor, XYcoor_i(j,:));
