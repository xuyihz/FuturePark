...
XYcoor_i = zeros(sideColu_num,2);   % 内筒XoY坐标第1(X)、2(Y)列。
XYcoor_o = zeros(sideColu_num*2,2); % 外表皮XoY坐标第1(X)、2(Y)列。

stairXY = [stairL/2, stairW/2; -stairL/2, stairW/2; -stairL/2, -stairW/2; stairL/2, -stairW/2]; % 原始坐标，未旋转，未转到整体坐标系
for i = 1:sideColu_num   % 尝试向量化
    [XYcoor_i(i,1), XYcoor_i(i,2)] = coorTrans(stairXY(i,1), stairXY(i,2), Deg_side); % 内筒
end
...
for i = 1:sideColu_num*2   % 尝试向量化
    [XYcoor_o(i,1), XYcoor_o(i,2)] = coorTrans(stairXY2(i,1), stairXY2(i,2), Deg_side); % 外筒
end
% 局部坐标系 转换至 整体坐标系
...
for i = 1:lengthlevelZaxis  % length(A(:)) A向量元素个数
    for k = 1:2 % 内筒，外筒
        if k == 1 % 节点编号规则：从0度角开始逆时针；先每层内筒，再每层外筒；从下到上。
            for j = 1:sideColu_num % 内部4个柱子
                iNO = iNO+1;
                fprintf(fileID,'   %d, %.4f, %.4f, %.4f\n',...
                    iNO,XYcoor_i(j,1),XYcoor_i(j,2),levelZaxis(i));
            end
        else
            for j = 1:sideColu_num*2 % 外部8个柱子
                iNO = iNO+1;
                fprintf(fileID,'   %d, %.4f, %.4f, %.4f\n',...
