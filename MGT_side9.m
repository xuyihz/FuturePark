...
sideRadius = 3250;
Corner_coor = Roof_boundary(1,:); % 角点

[X_temp, Y_temp, ~] = coorLxC_sym(CoC_side, sideRadius, Corner_coor, Roof_boundary(7,:)); %右边第一点 % [X, Y, Len] = coorLxC_sym(C0, R, P1, P2);
if X_temp(1) > X_temp(2)
    XYcoor_1 = [X_temp(1),Y_temp(1)];
else
    XYcoor_1 = [X_temp(2),Y_temp(2)];
end
[X_temp, Y_temp, ~] = coorLxC_sym(CoC_side, sideRadius, Corner_coor, Roof_boundary(2,:)); %上面边第一点 % [X, Y, Len] = coorLxC_sym(C0, R, P1, P2);
if X_temp(1) < X_temp(2)
