...
%%
function [iNO_end, iEL_end] = MGT_facade(fileID, iNO, iEL, car_num, CoC_tower, Deg_tower, tube_innerR, levelZaxis, levelPstart, iNO_towerS_init)    %注意这里的levelPstart是1x2数组
%% NODE
...
XYcoor_o3 = zeros(lengthlevelZaxis,car_num*2,2);	% 外筒XoY坐标第1(X)、2(Y)列。注意这里是三维数组。(Z方向幕墙有变化)
...
for j = levelPstart1:lengthlevelZaxis
    ...
    for i = 0:(car_num-1)   % 尝试向量化 % 旋转局部角度+整体角度
        [XYcoor_i(i+1,1), XYcoor_i(i+1,2)] = coorTrans(XYcoor_i_1(1), XYcoor_i_1(2), -car_num2pi*i+Deg_tower);       % 内筒点坐标 % 已旋转整体角度
        [XYcoor_o3(j,i*2+1,1), XYcoor_o3(j,i*2+1,2)] = coorTrans(XYcoor_o_1(1,1), XYcoor_o_1(1,2), -car_num2pi*i+Deg_tower); % 外筒点坐标1
        [XYcoor_o3(j,i*2+2,1), XYcoor_o3(j,i*2+2,2)] = coorTrans(XYcoor_o_1(2,1), XYcoor_o_1(2,2), -car_num2pi*i+Deg_tower); % 外筒点坐标2
    end
end
% 局部坐标系 转换至 整体坐标系
...
lengthXYcoor2 = length(XYcoor_i(:))/2 + length(XYcoor_o3(1,:,:))/2;  % 每层节点数备份
lengthXYcoor_f = length(XYcoor_o3(1,:,:))/2;  % 幕墙每层节点数

for i = levelPstart1:lengthlevelZaxis  % length(A(:)) A向量元素个数
    for j = 1:car_num
        iNO = iNO+1;
...
ELE_iPRO = 1;
iNO = iNO_init; % 初始化iNO
for i = levelPstart2:(lengthlevelZaxis-1)	% length(A(:)) A向量元素个数 % levelPstart 第几层开始停车，即下几层开敞
